...
def _do_setup( try_to_compile=False ):
    """
    wrap the setup call so that the FORTRAN compile can be made optional.
    """
    ...
    if try_to_compile:
        ...
        # will include optional named input parameter
        optD = {'ext_modules' : [ext_fprog]}
    else:
        optD = {}
    ...
    setup(
        ...
        #ext_modules = inp_ext_modules, # may be in optD
        ...
        #    ],
        #},
    **optD) # may include optional named parameter

if 'build' in sys.argv:
    print('Running setup... COMPILING FORTRAN')
    _do_setup( try_to_compile=True )
else:
    print('Running setup WITHOUT compiling FORTRAN')
    _do_setup( try_to_compile=False )
    
    try:
        # see if compiled module is available.
        from genericf2py import fprog
    except:
        print('WARNING... COMPILED FORTRAN FAILED... ATTEMPTING COMPILE')
        print('Re-Running setup... COMPILING FORTRAN')
        _do_setup( try_to_compile=True )
